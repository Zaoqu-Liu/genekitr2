...
plotEnrich <- function(enrich_df,
  ...
  if (!compare_group & !all_go) {
    if (any(duplicated(enrich_df$Description))) {
      ...
      warning(paste(
        "Checked duplicated terms in Description column, such as:",
    ...
    if (any(duplicated(enrich_df$Description))) {
      dup_term <- enrich_df$Description[duplicated(enrich_df$Description)]
      warning(paste(
        "Checked duplicated terms exist in Description column, such as:",
        paste0(utils::head(dup_term, 3), collapse = "|", "\n", "please make them unique then plot")
      ))
    }
    enrich_df <- enrich_df %>%
  ...
  if (plot_type == "wordcloud") {
    ...
    tdm <- tm::TermDocumentMatrix(x, control = list(
  ...
  if (plot_type == "goheat") {
    ...
    anno <- data.frame(
      ParentTerm = factor(reducedTerms[match(
        rownames(simMatrix),
        reducedTerms$go
      ), "parentTerm"]),
...
get_sim_data <- function(enrich_df, org = NULL, ont = NULL, sim_method) {
  org_name <- genekitr2::biocOrg_name$short_name
  ...
  if (is.null(org)) {
    tryCatch(
      ...
      error = function(e) {
        message("Please rename the ID column with organism short name from genekitr2::biocOrg_name!\nOR you can use 'JC' method")
      }
    )
  } else if (!tolower(org) %in% org_name) {
    stop("Please rename the ID column with organism short name from genekitr2::biocOrg_name!\nOR you can use 'JC' method.")
  }
