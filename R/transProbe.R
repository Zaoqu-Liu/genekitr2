...
transProbe <- function(id,
  ...
  if (is.null(platform)) {
    ...
    message(paste0(
      ...
      "\nIf not, please specify the platform from `genekitr2::", org, "_probe_platform`..."
    ))
  } else if (!tolower(platform) %in% colnames(probe_dat)) {
    stop(paste0("Please select platform from `genekitr2::", org, "_probe_platform`"))
  } else {
...
get_bioc_probe <- function(id, to_type, bioc_pkg) {
  ...
  if (!requireNamespace(bioc_pkg, quietly = TRUE)) {
    ...
    message(paste0(
      "Please firstly install ", bioc_pkg, ":\n",
      'BiocManager::install("', bioc_pkg, '")'
    ))
